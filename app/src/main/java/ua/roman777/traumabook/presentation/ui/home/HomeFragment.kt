...
import ua.roman777.traumabook.presentation.application.*
...
import ua.roman777.traumabook.presentation.enums.PatientListItemElement
import ua.roman777.traumabook.presentation.utils.imageUtil.ImagePathMaker
import ua.roman777.traumabook.presentation.utils.ImageFilePathDeterminer
...
import ua.roman777.traumabook.presentation.utils.activityContracts.AddPhotoFromCameraContract
import ua.roman777.traumabook.presentation.utils.diffUtilCallbacks.PatientDiffUtilCallback
import ua.roman777.traumabook.presentation.utils.setOnClickListener
...
class HomeFragment : Fragment() {
    ...
    private val imagePathMaker by lazy { ImagePathMaker() }
    ...
    private val getGalleryImageContracts =
        ...
        data?.let{uris ->
            lifecycleScope.launch {
                ...
                homeViewModel.addPatientPhoto(pathList.toList())
            }
        }
    }
        ...
        data?.let{ it ->
            lifecycleScope.launch {
                ...
                if (it.getIntExtra(INT_RESULT, Activity.RESULT_CANCELED) == Activity.RESULT_OK) {
                    ...
                    Timber.d(
                        "takeCameraImageContracts. registerForActivityResult(). photoPath = %s",
                        photoPath
                    )
                    homeViewModel.addPatientPhoto(photoPath)
                }
            }
        }
    }
    ...
    private  val createNewPatientContract =
        registerForActivityResult(ActivityResultContracts.TakePicture()) { isSuccess ->
            ...
            if (isSuccess) {
                lifecycleScope.launch {
                    homeViewModel.addPatient(imagePathMaker.cameraImagePath)
                }
            }
        }
    ...
    private fun addFromCamera(){
        lifecycleScope.launchWhenStarted {
            imagePathMaker.getFromCamera(requireContext()).let { uri ->
                val intent = Intent()
                intent.putExtra(CAMERA_FILE_STRING_URI, uri.toString())
                intent.putExtra(PHOTO_PATH, imagePathMaker.cameraImagePath)
